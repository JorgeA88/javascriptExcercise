...
var vegetables = ['eggplant', 'broccoli', 'carrot', 'cauliflower', 'zuccchini'];
...
function hasVowels(value){
  for (var i = 0; i < vowels.length; i++) {
      if (value.toLowerCase().includes(vowels[i])) {
       return true
     }
  }
  return false
...
function startsWithVowel(value){
  ...
  for (var i = 0; i < vowels.length; i++) {
      if (value.toLowerCase() === vowels[i]){
  return value.startswitch(vowels)
      }
}}
...
function startsAndEndsWithVowel(str) {
  ...
  var vowels = ("aeiouAEIOU");
  for (var i = 0; i < vowels.length; i++) {
      if (str.toLowerCase() == vowels[i]){
  return str.startswitch(vowels)
  
}};
  if (typeof(str) === "string" && str.toLowerCase() == vowels[i]) {
    return str.endsWith()
  }
} 
...
function maxMinusMin([num1,num2,num3]){
...
// Write a function definition named getSmallestNumber that takes in sequence of numbers and returns the smallest number.
  ...
  function getSmallestNumber(value1,value2,value3){
  if(isNaN(value1) || isNaN(value2) || isNaN(value3)){
    return false
}else{
